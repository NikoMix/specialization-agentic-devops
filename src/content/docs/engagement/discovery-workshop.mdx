...
<Tabs>
  <TabItem label="WAF">
    Capture answers for the [WAF Assessment](../waf-assessment/) on Operational Excellence + Security pillars during the current-state walkthrough.
  </TabItem>
  <TabItem label="MAP / readiness">
    Capture answers for the [Assessment Platform Inputs](../assessment-platform-inputs/) (DevOps maturity, Copilot readiness, GHAS posture) during the baseline tour.
  </TabItem>
</Tabs>
...
<Aside type="tip" title="Download the workfiles">
  <a href={`${import.meta.env.BASE_URL}templates/engagement/discovery-workshop-deck.pptx`}>📄 Discovery Workshop Deck (PPTX)</a> · <a href={`${import.meta.env.BASE_URL}templates/engagement/discovery-workshop-workbook.xlsx`}>📄 Discovery Workshop Workbook (XLSX)</a>
</Aside>
...
<Aside type="tip">
  ...
  and the MDX. Use the `lesson-learned` issue template to capture the rationale.
</Aside>
